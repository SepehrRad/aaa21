...
def census_tract_to_hexagon(
    ...
    for polygon in geometry:
        ...
        hexagons = h3.polyfill(
            polygon.__geo_interface__, res=res, geo_json_conformant=True
        )
...
def polygon_to_hexagon(geometry, res=9):
    ...
    hex_gdf["hex"] = hex_gdf.apply(
        lambda x: h3.polyfill(
            x["geometry"].__geo_interface__, res=res, geo_json_conformant=True
        ),
        axis=1,
    )
    ...
    hex_gdf = hex_gdf.drop_duplicates("hex")
    hex_gdf["geometry"] = hex_gdf.apply(
        lambda x: Polygon(h3.h3_to_geo_boundary(x["hex"], True)), axis=1
    )
    return hex_gdf
