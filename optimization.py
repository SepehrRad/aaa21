...
def add_optimization_results_to_data_set(
    df, geometry_map, hexagon_map, optimization_problem
):
    """
...
def _generate_random_points_in_polygon(polygon):
    ...
    while True:
        random_point = Point(
            random.uniform(float(min_x), float(max_x)),
            random.uniform(float(min_y), float(max_y)),
        )
...
def show_stations_on_map(df, geo_json):
    ...
    folium.Choropleth(
        geo_data=geo_json, name="choropleth", fill_color="gray", fill_opacity=0.1
    ).add_to(base_map)
    ...
    for index, row in df.iterrows():
        if row["Level 2 Station"] > 0:
            ...
            for i in range(number_of_stations):
                p1, p2, p3, p4, p5, p6, p7 = row["hex_6_geometry_pickup"].get(
                    "coordinates"
                )[0]
                ...
                folium.Marker(
                    ...
                    tooltip="Level 2 Station",
                    icon=folium.Icon(prefix="fa", color="green", icon="car"),
                ).add_to(base_map)
        ...
        if row["Level 3 Station"] > 0:
            ...
            for i in range(number_of_stations):
                ...
                folium.Marker(
                    (station_loc.x, station_loc.y),
                    popup=f"Expected Daily Taxi Demand:{row['Expected Daily Taxi Demand']}",
                    tooltip="Level 3 Station",
                    icon=folium.Icon(prefix="fa", color="red", icon="car"),
                ).add_to(base_map)
    ...
    legend_2 = folium.FeatureGroup(
        name=f"<span style=color:{'green'}>{'Level 2 Station'}</span>"
    )
    base_map.add_child(legend_2)
    legend_3 = folium.FeatureGroup(
        name=f"<span style=color:{'red'}>{'Level 3 Station'}</span>"
    )
