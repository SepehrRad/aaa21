...
def _add_weather_data(df, weather_df, temporal_resolution):
    ...
    Doc String!
    """
    _ = weather_df.groupby(
        [
            pd.Grouper(
                key="Trip Start Timestamp",
                freq=temporal_resolution,
                offset=f"{'23h00min' if temporal_resolution == '6H' else '00h00min'}",
            )
        ]
    ).agg(
        {
...
def create_aggregated_data(
    ...
    df = _add_weather_data(
        df=df, weather_df=weather_df, temporal_resolution=temporal_resolution
    )
    return df
